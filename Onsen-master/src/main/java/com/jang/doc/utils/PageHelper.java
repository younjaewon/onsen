...
public class PageHelper {
	...
	public StringBuffer getPageUrl(SearchVO searchVO, int totalRow) {
		...
		if (currentBlock > 1) {
			pageHtml.append("<span><a href=\"list?page=" + (currentPage - 1) +"&type="+type+"&keyword="+key + "\"><이전></a>&nbsp;&nbsp;");
		} else {
		...
		for (int i = startPage; i <= endPage; i++) {
			if (i == currentPage) {
				...
				pageHtml.append(".&nbsp;<a href=\"list?page=" + i +"&type="+type+"&keyword="+key+ "\" class=\"page\">" + i + "</a>&nbsp;");
			}
		}
		// 현재 블록 안이 전체 블럭 보다 작으면 [다음] 표시
		if (currentBlock < totalBlock) {
			pageHtml.append(".&nbsp;&nbsp;<a href=\"list?page=" + (currentPage + 1) +"&type="+type+"&keyword="+key+ "\"><다음></a></span>");
		} else {
			pageHtml.append(".</span>");
		}
